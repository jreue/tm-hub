...
void DisplayController::renderSystemResults() {
  ...
  for (size_t i = 0; i < count; ++i) {
    renderSystemResultItem(results[i].label.c_str(), results[i].value.c_str(),
                           startY + i * lineHeight);
  }
}

void DisplayController::renderSystemResultItem(const char* label, const char* value, int32_t y) {
  // Clear the line
